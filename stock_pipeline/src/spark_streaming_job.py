...
Luồng dữ liệu:
  Kafka (raw OHLCV)
    -> parse & tính EMA5 (dựa trên history buffer)
    -> scale bằng scaler offline
    -> dự đoán close (dùng seq_len row liền trước)
...
# Import module nội bộ (giữ nguyên từ offline pipeline)
...
INCREMENTAL_LR    = 1e-4   # Learning rate nhỏ để tránh catastrophic forgetting
TRIGGER_INTERVAL  = "5 seconds"
EMA_SPAN          = 5
EMA_SEED_ROWS     = 30     # Số rows lịch sử dùng làm "warm-up" tính EMA5
...
INPUT_SIZE     = config["input_size"]       # = 6
HIDDEN_SIZE    = config["hidden_size"]
FEATURES_COLS  = config["features_cols"]    # ['open','high','low','close','volume','EMA5']
TARGET_COL_IDX = config["target_col_idx"]   # = 3 (close)
...
# Tải lịch sử để khởi tạo context
df_history = pd.read_parquet(f"{ASSETS_DIR}/historical_data.parquet")

# ── 3a. Close-price buffer: dùng để tính EMA5 cho dữ liệu mới ──────────────
# Seed bằng EMA_SEED_ROWS dòng cuối của lịch sử (chưa scale)
close_history: deque = deque(
    df_history["close"].tail(EMA_SEED_ROWS).tolist(),
    maxlen=EMA_SEED_ROWS + 200,
)

# ── 3b. Sequence buffer: lưu các row đã scale để tạo input cho model ────────
# Seed bằng SEQ_LEN dòng cuối của lịch sử (đã scale)
_hist_scaled = scaler.transform(df_history[FEATURES_COLS].values)
seq_buffer: deque = deque(
    _hist_scaled[-SEQ_LEN:].tolist(),
    maxlen=SEQ_LEN * 10,  # giữ tối đa SEQ_LEN*10 để tránh tràn RAM
)

# ── 3c. Accumulator: tích lũy raw data mới upload MinIO sau mỗi batch ───────
new_rows_accumulator: list = []

log.info(f"Đã seed buffer: close={len(close_history)} rows, seq={len(seq_buffer)} rows.")
...
def compute_ema5(close_list: list) -> float:
    """
    Tính EMA5 từ chuỗi close price, trả về giá trị EMA mới nhất.
    Dùng adjust=False để match với pandas truyền thống.
    """
...
def predict(x_seq: list) -> float:
    """
    Dự đoán giá close (đã scale) từ chuỗi x_seq có dạng (SEQ_LEN, INPUT_SIZE).
...
def inverse_close(scaled_val: float) -> float:
    """
    Inverse transform chỉ cho cột close (TARGET_COL_IDX).
    Tạo dummy vector để dùng scaler.inverse_transform.
    """
...
# Optimizer cho incremental update – khởi tạo 1 lần, tái sử dụng mọi batch
...
def incremental_update(x_seq: list, y_true_scaled: float) -> float:
    """
    Fine-tune model trên 1 sample mới vừa quan sát được.
    x_seq    : (SEQ_LEN, INPUT_SIZE) – window liền trước row hiện tại
    y_true_scaled : giá trị close thực tế đã scale của row hiện tại
    Trả về loss.
    """
    model.train()
    x = torch.tensor(x_seq, dtype=torch.float32).unsqueeze(0).to(device)
...
def save_model_to_minio(tag: str = "updated"):
    """Lưu trọng số model hiện tại lên MinIO."""
...
def flush_new_data_to_minio():
    """Upload dữ liệu stream mới tích lũy trong batch lên MinIO dạng parquet."""
...
def process_batch(batch_df, batch_id: int):
    """
    Được gọi mỗi micro-batch từ Spark Structured Streaming.

    Với mỗi row đến (theo thứ tự thời gian):
      1. Tính EMA5 từ close_history buffer.
      2. Scale toàn bộ feature vector.
      3. Nếu seq_buffer có đủ SEQ_LEN rows (= dữ liệu trước đó),
         dự đoán close của row hiện tại.
      4. So sánh prediction vs actual, incremental update model.
      5. Append scaled row vào seq_buffer.
      6. Ghi kết quả vào PostgreSQL.
      7. Tích lũy raw row vào accumulator.
    Sau khi xử lý hết batch:
    ...
    # Sort theo date để đảm bảo thứ tự thời gian trong batch
    ...
    for row in rows:
        try:
            ...
            # ── Bước 3 & 4: Dự đoán + Incremental Update ─────────────────────
            close_predicted = None
            inc_loss        = None

            if len(seq_buffer) >= SEQ_LEN:
                # Lấy SEQ_LEN row cuối của buffer (TRƯỚC khi thêm row hiện tại)
                ...
                # Incremental update: dùng true label (close_ thực tế) vừa nhận
                ...
                # Buffer chưa đủ dữ liệu (giai đoạn khởi động)
                log.warning(
                    f"  [{date_str}] Buffer {len(seq_buffer)}/{SEQ_LEN} – "
                    f"chưa đủ để dự đoán."
                )
...
spark = (
    ...
    # Giảm số partition để tránh overhead khi xử lý từng row một
...
# ──────────────────────────────────────────────
# 8. ĐỊNH NGHĨA SCHEMA KAFKA MESSAGE
# ──────────────────────────────────────────────
# Phải khớp với cấu trúc JSON mà Kafka Producer gửi
...
raw_stream = (
    ...
    # "latest": chỉ đọc tin nhắn MỚI (bỏ qua lịch sử trong topic)
    # Đổi thành "earliest" nếu muốn replay toàn bộ topic
    .option("startingOffsets", "latest")
    .option("failOnDataLoss", "false")
    .load()
)

# Kafka trả về cột "value" dạng binary -> cast sang string -> parse JSON
parsed_stream = (
    raw_stream
    .select(
    ...
    .select("data.*")
    # Lọc bỏ các row có giá trị null (do parse lỗi)
...
try:
    ...
    # Lưu model đã incremental update lên MinIO để retrain offline sau phiên
    ts  = datetime.now().strftime("%Y%m%d_%H%M%S")
    save_model_to_minio(tag=f"incremental_{ts}")

    # Flush nốt dữ liệu còn trong accumulator (nếu có)
    flush_new_data_to_minio()
